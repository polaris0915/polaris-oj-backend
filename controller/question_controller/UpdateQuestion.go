...
func (qc *QuestionController) UpdateQuestion(c *gin.Context) {
	...
	// 1. 绑定请求数据到DTO层模型中
	var requestDto dto.RequestDto[*allModels.Question] = new(question_dto.QuestionUpdateRequest)
	/*
	...
	*/
	...
	if response.Err = qc.questionService.UpdateQuestion(session, requestDto, nil); response.Err != nil {
